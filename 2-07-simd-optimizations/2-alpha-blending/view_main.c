...
bool prepare_notes_texture(
	...
	static char const * const NOTES[] = {
		"Есть три режима: обычный, SSE и AVX.",
		"Если режим не поддерживается, при",
		"его активации вы получите сообщение.",
		"Tab      -- переключить режим.",
		"Alt      -- вкл/выкл отображение",
		"текстов."
	};
...
int main() {
	...
	SDL_Window* window = SDL_CreateWindow("Наложение картинок (alpha-blending)", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, SCREEN_COLS, SCREEN_ROWS, SDL_WINDOW_SHOWN);
	if (window == NULL) {
	...
	static const int NOTES_FONT_SIZE = 15; // In points.
	...
	while (!exiting) {
		...
		switch (computation_mode) {
			case COMPUTATION_MODE_PLAIN: {
				blend_nosse(pixels, blend_pictures);
				break;
			}

			case COMPUTATION_MODE_SSE: {
				blend_sse(pixels, blend_pictures);
				break;
			}

			case COMPUTATION_MODE_AVX: {
				blend_avx(pixels, blend_pictures);
				break;
			}
		}
	...
	blend_pictures = blend_pictures_delete(blend_pictures);
