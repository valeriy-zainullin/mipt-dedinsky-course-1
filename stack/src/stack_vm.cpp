...
bool STACK_INIT_FUNCTION_NAME(Variable definition, STACK_TYPE_NAME* stack_ptr, void* address, size_t capacity) {
	...
	*stack_ptr = (STACK_TYPE_NAME*) ((char*) address - (sizeof(StackImpl) - sizeof(uint32_t)));

	StackImpl* stack_impl_ptr = (StackImpl*) *stack_ptr;
	memset(stack_impl_ptr, 0, sizeof(StackImpl));
	stack_impl_ptr->size = 0;
	stack_impl_ptr->capacity = capacity;
